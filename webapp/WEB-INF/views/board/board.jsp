...
<body>
	<div class="board_wrap">
		...
		<div class="bt_wrap">
			<a href="board.go" class="count" id="recently_a">최신순</a>
			<a href="board.count.go" class="count" id="cnt_a">조회순</a>
		...
		<div class="board_list_wrap">
			...
			<div class="board_page">
				...
				<c:if test="${startNum != 1 }">
					<a href="board.page.change?p=${curPage-5 }" class="bt prev">&lt;&lt;</a>
				</c:if>
				<c:if test="${curPage != 1 }">
				...
				<c:if test="${curPage <= allPageCount }">
					<c:forEach var="i" begin="0" end="4">         
	               		<c:if test="${startNum+i <= lastNum }">
	               			<a href="board.page.change?p=${startNum+i}" class = num>${startNum+i }</td>
	               		</c:if>
	            	</c:forEach>
            	</c:if>
				<c:if test="${curPage < allPageCount }">
					<a href="board.page.change?p=${curPage+1 }" class="bt next">&gt;</a>
					<a href="board.page.change?p=${curPage+5 }" class="bt next">&gt;&gt;</a>
				</c:if>
			</div>
		</div>
	</div>
