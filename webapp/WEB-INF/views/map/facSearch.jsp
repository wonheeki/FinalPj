...
<!-- 지도를 표시할 div 입니다 -->
   ...
   키워드 : <input id="keyword" size="15"> 
    <button id="mapSearch" type="button">검색하기</button>
     -->
   <div id="map_wrap">
		<a href="map.go" id="facBtn">공공시설</a> 
		<a href="map.sub.go"class="mapbtn">지하철</a> 
		<a href="map.charge.go" class="mapbtn">휠체어충전소</a>
...
$(function(){
   ...
   $.getJSON('facilities.get', function(fg){
		$.each(fg.facilities, function(i, f){
	        ...
	        '        <div class="body">' + 
	        '            <div class="desc">' + 
	        '                <div class="ellipsis">'+f.f_FCLT_KIND_NM+'</div>' + 
	        '                <div class="jibun ellipsis">'+f.f_FCLT_ADDR+'</div>' +
	        '            </div>' + 
	        '        </div>' + 
	        '    </div>' +    
	        '</div>';
			...
			kakao.maps.event.addListener(markers, 'click', function() {
				location.href="facMap.View.go?f_no=" + f.f_no;
	      	});
			
			$("#mapSearch").click(function(){
				markers.setMap(null);
				let search = $("#keyword").val();
				$.getJSON("fac.search?f_fclt_nm=" + search, function(fg){
					$.each(fg.facilities, function(i, f){
